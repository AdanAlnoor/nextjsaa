...
import type { Division, Section, Assembly } from '@/types/library'
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Create a new {currentLevelInfo?.label.toLowerCase()} in the library hierarchy
            {parentName && ` under "${parentCode} - ${parentName}"`}
          </DialogDescription>
          {isParentLocked && parentCode && (
            <div className="bg-blue-50 border border-blue-200 rounded-md p-3 mt-2">
              <div className="flex items-center gap-2 text-sm text-blue-800">
                <Badge variant="secondary" className="text-xs">Context</Badge>
                <span>Adding to: <code className="bg-blue-100 px-1 rounded">{parentCode}</code> - {parentName}</span>
              </div>
            </div>
          )}
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-muted/50 p-4 rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              <Badge variant="outline">
                Level {targetLevel}: {currentLevelInfo?.label}
              </Badge>
              <Badge variant="secondary">
                Format: {currentLevelInfo?.format}
              </Badge>
            </div>
            {parentCode && (
              <p className="text-sm text-muted-foreground">
                Parent: {parentCode} - {parentName}
              </p>
            )}
          </div>
          ...
          {targetLevel > 1 && (
            <div className="space-y-4">
              <h3 className="font-medium">Parent Selection</h3>
              
              {targetLevel >= 2 && (
                <div>
                  <Label htmlFor="division">
                    Division
                    {isParentLocked && parentLevel >= 1 && (
          ...
          {/* Basic Information */}
          <div className="space-y-4">
            <h3 className="font-medium">{currentLevelInfo?.label} Information</h3>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  placeholder={`${currentLevelInfo?.label} name`}
                />
              </div>

              <div>
                <Label htmlFor="code">
                  Code <span className="text-sm text-muted-foreground">(auto-generated if empty)</span>
                </Label>
                <Input
                  id="code"
                  value={formData.code}
                ...
                {previewCode && !formData.code && (
                  <p className="text-xs text-muted-foreground mt-1">{previewCode}</p>
                )}
              </div>
            </div>
