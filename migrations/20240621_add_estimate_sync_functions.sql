-- Migration: Add estimate sync functions
-- Description: Adds PostgreSQL functions to sync estimate data to cost control

-- Make sure the cost_control_items table has the needed columns
DO $$
BEGIN
    -- Add columns if they don't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'cost_control_items' AND column_name = 'estimate_item_id') THEN
        ALTER TABLE cost_control_items ADD COLUMN estimate_item_id UUID REFERENCES estimate_items(id) ON DELETE SET NULL;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'cost_control_items' AND column_name = 'is_deleted') THEN
        ALTER TABLE cost_control_items ADD COLUMN is_deleted BOOLEAN DEFAULT FALSE;
    END IF;
END
$$;

-- Create function to sync estimate data to cost control
...
-- Create trigger function to automatically sync when estimate items change
...
CREATE OR REPLACE FUNCTION trigger_sync_estimate_to_cost_control_on_delete()
RETURNS TRIGGER AS $$
BEGIN
  -- Mark corresponding cost control item as deleted
  UPDATE cost_control_items
  SET is_deleted = TRUE
  WHERE estimate_item_id = OLD.id;
...
DROP TRIGGER IF EXISTS estimate_items_delete_trigger ON estimate_items;
CREATE TRIGGER estimate_items_delete_trigger
AFTER DELETE ON estimate_items
FOR EACH ROW
EXECUTE FUNCTION trigger_sync_estimate_to_cost_control_on_delete(); 